...
class ReviewAnalysisService {
  ...
  private readonly painPointKeywords = {
    service: [
      'slow service', 'rude staff', 'unfriendly', 'poor service', 'bad service',
      'long wait', 'waited forever', 'ignored', 'unprofessional', 'attitude',
      'no attention', 'dismissive', 'unhelpful'
    ],
    food: [
      'cold food', 'overcooked', 'undercooked', 'tasteless', 'bland', 'stale',
      'poor quality', 'not fresh', 'burnt', 'soggy', 'dry', 'greasy',
      'small portions', 'overpriced', 'expensive'
    ],
    operations: [
      'dirty', 'unclean', 'messy', 'disorganized', 'chaotic', 'loud',
      'cramped', 'uncomfortable', 'broken', 'out of order', 'cash only',
      'no card', 'payment issues', 'system down'
    ],
    ordering: [
      'wrong order', 'missing items', 'incorrect', 'forgot', 'mixed up',
      'no online ordering', 'cant order online', 'phone busy', 'hard to reach'
    ]
  };

  private readonly positiveKeywords = {
    service: [
      'excellent service', 'friendly staff', 'attentive', 'professional',
      'helpful', 'courteous', 'welcoming', 'great service', 'amazing staff'
    ],
    food: [
      'delicious', 'amazing food', 'excellent food', 'fresh', 'tasty',
      'flavorful', 'perfect', 'outstanding', 'incredible', 'best'
    ],
    operations: [
      'clean', 'organized', 'efficient', 'quick', 'fast', 'smooth',
      'easy ordering', 'convenient', 'modern', 'updated'
    ]
  };
  ...
  private extractPainPoints(reviews: ReviewData[]): string[] {
    ...
    reviews.forEach(review => {
      if (review.rating <= 3) { // Focus on negative/neutral reviews
        ...
        this.painPointKeywords.service.forEach(keyword => {
          if (text.includes(keyword)) {
            painPointCounts['Poor customer service and staff attitude'] = 
              (painPointCounts['Poor customer service and staff attitude'] || 0) + 1;
          }
        });
        ...
        this.painPointKeywords.operations.forEach(keyword => {
          if (text.includes(keyword)) {
            painPointCounts['Operational and cleanliness concerns'] = 
              (painPointCounts['Operational and cleanliness concerns'] || 0) + 1;
          }
        });
  ...
  private extractCommonComplaints(reviews: ReviewData[]): string[] {
    ...
    reviews.forEach(review => {
      if (review.rating <= 2) { // Focus on very negative reviews
        const text = review.text.toLowerCase();
        
        // Specific complaint patterns
        if (text.includes('long wait') || text.includes('slow') || text.includes('waited')) {
          complaints['Long waiting times'] = (complaints['Long waiting times'] || 0) + 1;
        }
        ...
        if (text.includes('small portion') || text.includes('tiny') || text.includes('not enough')) {
          complaints['Small portion sizes'] = (complaints['Small portion sizes'] || 0) + 1;
        }
        
        if (text.includes('cash only') || text.includes('no card') || text.includes('payment')) {
          complaints['Limited payment options'] = (complaints['Limited payment options'] || 0) + 1;
        }
        
        if (text.includes('parking') || text.includes('no parking')) {
          complaints['Parking issues'] = (complaints['Parking issues'] || 0) + 1;
        }
      }
    });
  ...
  private extractOperationalIssues(reviews: ReviewData[]): string[] {
    const issues: string[] = [];
    const issuePatterns = [
      { pattern: /cash only|no card|payment.*problem/i, issue: 'Limited payment methods - opportunity for modern POS system' },
      { pattern: /no.*online.*order|cant.*order.*online/i, issue: 'No online ordering system - digital transformation opportunity' },
      { pattern: /system.*down|pos.*not.*work|machine.*broken/i, issue: 'POS system reliability issues - upgrade opportunity' },
      { pattern: /long.*queue|slow.*service|waited.*long/i, issue: 'Service efficiency issues - workflow optimization needed' },
      { pattern: /wrong.*order|mixed.*up|forgot.*item/i, issue: 'Order accuracy problems - better order management needed' },
    ];

    const reviewTexts = reviews.map(r => r.text).join(' ').toLowerCase();
  ...
  private calculateSentimentScores(reviews: ReviewData[]): {
    ...
    reviews.forEach(review => {
      ...
      if (text.includes('food') || text.includes('taste') || text.includes('meal') || text.includes('dish')) {
        foodScore += review.rating;
        foodCount++;
      }
      
      // Service-related mentions
      if (text.includes('service') || text.includes('staff') || text.includes('waiter') || text.includes('server')) {
        serviceScore += review.rating;
        serviceCount++;
      }
      
      // Ambiance-related mentions
      if (text.includes('atmosphere') || text.includes('ambiance') || text.includes('place') || text.includes('environment')) {
    ...
    return {
      ...
      food: foodCount > 0 ? Math.round((foodScore / foodCount) * 10) / 10 : overall,
      service: serviceCount > 0 ? Math.round((serviceScore / serviceCount) * 10) / 10 : overall,
      ambiance: ambianceCount > 0 ? Math.round((ambianceScore / ambianceCount) * 10) / 10 : overall,
    };
  }
  ...
  generateSalesInsights(analysis: ReviewAnalysisResult): {
    ...
    analysis.painPoints.forEach(painPoint => {
      if (painPoint.includes('service')) {
        opportunities.push('Staff training and customer service improvement through better POS workflow');
      }
      if (painPoint.includes('order') || painPoint.includes('system')) {
        opportunities.push('Order accuracy improvement with modern POS system and kitchen display');
      }
      if (painPoint.includes('payment')) {
        opportunities.push('Multiple payment options and faster checkout process');
      }
    });
    ...
    analysis.commonComplaints.forEach(complaint => {
      if (complaint.includes('expensive') || complaint.includes('price')) {
        objections.push({
          objection: "We're already struggling with costs, we can't afford new systems",
          response: "I understand cost is a concern. Our POS system actually helps reduce costs through better inventory management, reduced waste, and faster service that increases table turnover. Most clients see ROI within 6 months."
        });
      }
      if (complaint.includes('cash only')) {
        objections.push({
          objection: "Our customers prefer cash, we don't need card payments",
          response: "While cash is important, offering multiple payment options can increase average order value by 15-20%. Our system handles both seamlessly, and you'll capture sales from customers who only have cards."
        });
      }
    });
